...
# Builds from the local repository and publishes to .\NetworkMonitoring
...
# Resolve install directory: -InstallDir, -path, --path=<dir>, or .\NetworkMonitoring
$PathFromArgs = $null
foreach ($arg in $args) {
    if ($arg -match '^(?:--|-)path=(.+)$') {
...
if (-not $InstallDir) {
    if ($PathFromArgs) {
        ...
        $InstallDir = Join-Path (Get-Location).Path "NetworkMonitoring"
    }
}
...
if (-not (Get-Command dotnet -ErrorAction SilentlyContinue)) {
    Write-Error-Exit ".NET SDK not found. Install .NET 9 SDK, then run install.ps1 again."
}

$LocalProject = Join-Path $PSScriptRoot "NetworkMonitor\NetworkMonitor.csproj"
if (-not (Test-Path $LocalProject)) {
    Write-Error-Exit "Project not found at $LocalProject. Run install.ps1 from the repository root."
}
